...
- (NSDictionary *)controllerPreferences {
    ...
    if (self.gamePlayDictionary == nil){
        ...
        NSString *preferenceFile = @"/var/mobile/Library/Preferences/com.nito.codmc.plist";
        NSString *localFile = [[self documentsFolder] stringByAppendingPathComponent:@"com.nito.codmc.plist"];
        NSFileManager *man = [NSFileManager defaultManager];
        if (![man fileExistsAtPath:localFile]){
            [man copyItemAtPath:preferenceFile toPath:localFile error:nil];
        }
        NSLog(@"CODMC: localFile: %@", localFile);
...
- (void)updateGamplayValue:(id)value forKey:(NSString *)theKey {
    ...
    if (theKey != nil && value != nil){
        ...
        NSString *preferenceFile = [[self documentsFolder] stringByAppendingPathComponent:@"com.nito.codmc.plist"];
...
- (NSInteger)panning:(CGFloat)val {
    ...
    if (sendIt) {
        if (val < 0)  {
            //NSLog(@"CODMC: pan left!");
            returnValue = 1;
        } else {
            //NSLog(@"CODMC: pan right!");
...
- (void)setupTapRecognizerIfNeeded {
    ...
    if (view != nil && _tapSetup == FALSE){
        ...
        _tapSetup = TRUE;
        NSLog(@"#### show alert??");
        [RKDropdownAlert title:@"CODMC 1.1-* Activated" message:@"Tap here now OR double tap anywhere on the screen with THREE fingers to bring up the control customization window." backgroundColor:[UIColor redColor] textColor:[UIColor whiteColor] time:3 delegate:self];
    }
}
   ...
   // profile.rightThumbstick.valueChangedHandler = ^(GCControllerDirectionPad * _Nonnull dpad, float xValue, float yValue){
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:.000482 repeats:TRUE block:^(NSTimer * _Nonnull timer) {
        ...
        if ([self experimentalMode]){
            ...
            CGPoint mid = CGPointMake(466,162);
            ...
            if (xValue == 0 && yValue == 0){
                ...
                if (CGPointEqualToPoint(self_weak_.previousRightPoint, CGPointZero)){ //not touching down
                    ...
                    NSLog(@"CODMC: first drag moving from %@ to %@", NSStringFromCGPoint(mid), NSStringFromCGPoint(self_weak_.previousRightPoint));
...
//                            }
                        ...
                        //if (!CGPointEqualToPoint(secondPoint, newPoint)){
                            NSLog(@"CODMC: second point %@ to %@", NSStringFromCGPoint(newPoint), NSStringFromCGPoint(secondPoint));
    ...
    profile.leftThumbstick.valueChangedHandler = ^(GCControllerDirectionPad * _Nonnull dpad, float xValue, float yValue) {
        ...
        if (xValue == 0 && yValue == 0){
            ...
            if (CGPointEqualToPoint(previousPoint, CGPointZero)){ //not touching down
                ...
                //NSLog(@"CODMC: already down, moving from %@ to %@", NSStringFromCGPoint(previousRightPoint),NSStringFromCGPoint(newPoint));
                /*
...
- (CGPoint)pointForActionType:(PGBActionType)type {
    ...
    */
    ...
    switch (type) {
        case kPGBActionTypeAim:
            outpoint = [self convertPointForScreen:CGPointMake(588,187)];
            break;
            
        case kPGBActionTypeRun:
            outpoint = [self convertPointForScreen:CGPointMake(513,113)];
            break;
        ...
        case kPGBActionTypeReload:
            outpoint = [self convertPointForScreen:CGPointMake(504,283)];
            break;
            
        case kPGBActionTypeFirstWeapon:
            outpoint = [self convertPointForScreen:CGPointMake(326,333)];
            break;
            
        case kPGBActionTypeSecondWeapon:
            outpoint = [self convertPointForScreen:CGPointMake(416,333)];
            break;
        ...
        case kPGBActionTypeRight:
            outpoint = [self convertPointForScreen:CGPointMake(567,258)];
            break;
        ...
        case kPGBActionTypeJump:
            outpoint = [self convertPointForScreen:CGPointMake(636,227)]; //long press
            break;
        ...
        case kPGBActionFirstItemSelect:
            outpoint = [self convertPointForScreen:CGPointMake(336,279)];
            break;
